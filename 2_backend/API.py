...
async def get_html_content():
    try:
        with open('../1_outputs/report_5.html', encoding='utf-8') as report_f:
...
async def model_predictif(index_cible: int) -> List[float] :
    ...
    file_path = "../1_outputs/base_futur_prep_algo.csv"
    separator = '\t'
    compression = 'gzip'
...
try:
    explanation = joblib.load("../1_outputs/shap_explanation_model_risk.joblib")
...
async def explain(index_cible: int):
    ...
    try:
        ...
        fig=shap.waterfall_plot(shap_values,max_display=20,show=False)
        ...
        plt.savefig('../1_outputs/temp.jpeg',bbox_inches='tight')
        plt.close(fig)
        
        return "../1_outputs/temp.jpeg"
...
async def explain():
    ...
    try: 
        ...
        plt.savefig('../1_outputs/temp_2.jpeg',bbox_inches='tight')
        plt.close()
        
        return "../1_outputs/temp_2.jpeg"
