...
if 'random_sel' not in st.session_state:
    ...
    response3=requests.get(endpoint_id_clients)
    if response3.status_code == 200:
            #créer l'objet serie_index_clients issus de la requete
            serie_index_clients=pd.read_csv(StringIO(response3.text),index_col='ID')
...
if selected_values:
    for selected_value in selected_values:
        #changement de nom pour coller avec définition de l'endpoint dans l'api
        index_cible=selected_value
        # accès aux données "ligne relative à l'ID sélectionné"
        endpoint_donnees_ID = f"{api_base_url}/data/{index_cible}"

        response2 = requests.get(endpoint_donnees_ID)

        # Vérif résussite de la requete
        if response2.status_code == 200:
            ligne_infos=pd.read_csv(StringIO(response2.text),index_col='ID')
        # ajout des infos de la requete au df "temp"
        temp =pd.concat([temp, ligne_infos])
    st.header("Informations générales",divider="orange")
    temp=temp.T
    
    # Affichage de temp dans le dashboard
...
# Requete pour le df total de la base sur les colonnes pour l'affichage
response = requests.get(endpoint_df_infos)

# Check if the request was successful
if response.status_code == 200:
    # Parse the JSON response from the API
    data =response.text
    df_infos=pd.read_csv(StringIO(data),index_col='ID')
